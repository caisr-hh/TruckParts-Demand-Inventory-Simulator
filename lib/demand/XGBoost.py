...
class XGBoost:
    def __init__(self, data_dir: str, feature_series):
        ...
        # XGBoost folder path
        self.hold_folder = os.path.join(self.data_dir, "XGBoost")
        ...
        for key in FEATURES_DICT:
            ...
            if not opt:
                if len(opt_params) == 0:
                    ...
                    reg = xgb.XGBRegressor(n_estimators=opt_params['n_estimators'], 
                                    early_stopping_rounds=50, 
                ...
                def objective(trial):
                    # searching range of hyperparamters
                    params = {
                        'max_depth': trial.suggest_int('max_depth', 1, 50),
                        'n_estimators': trial.suggest_int('n_estimators', 100, 1000),
                        'learning_rate': trial.suggest_loguniform('learning_rate', 0.01, 1.0), 
                        'min_child_weight': trial.suggest_loguniform('min_child_weight', 1, 20)
                    }
                    ...
                    if lossfunc_type == "RMSE":
                        ...
                        mean_mse = - score.mean()
                        func_val = mean_mse ** 0.5
                    # MAE
                    elif lossfunc_type == "MAE": 
                        score = model_selection.cross_val_score(
                            reg,
                            X_train,
                            y_train,
                ...
                reg = xgb.XGBRegressor(n_estimators=opt.best_params['n_estimators'], 
                                    early_stopping_rounds=50, 
                                    learning_rate=opt.best_params['learning_rate'],
                                    max_depth = opt.best_params['max_depth'],
                                    enable_categorical=True)
    ...
    # method: compute feature importance
