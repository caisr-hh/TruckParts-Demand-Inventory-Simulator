...
class IAE:
    def __init__(self, ground_truth, forecast):
        # ground_truth / forecast data
    ...
    # method: calculate ADI
    def calc_ADI(self):
        N = len(self.ground_truth)
        nz = (self.ground_truth > 0).sum()
        self.ADI = N/nz
    

    def get_level_of_alignment_for_one_demand_occurrence(self, ts, other, index, number_of_masks_to_use, mode, verbose=False):
    ...
    def get_total_level_of_alignment(self, ts, other, number_of_masks_to_use, total_ground_truth_val, mode, verbose=False):
        ...
        for index, val in enumerate(ts):
            if val != 0:
                alignment_error = self.get_level_of_alignment_for_one_demand_occurrence(ts=ts, other=other, index=index, number_of_masks_to_use=number_of_masks_to_use, mode=mode, verbose=verbose)
    ...
    def intermittent_alignment_error(self, mode='squared', timing_tolerance=0.5, verbose=False):
        assert len(self.ground_truth) == len(self.forecast), 'Ground truth and forecast have to be the same length'
        ...
        # Calculate the total ground truth value
        total_ground_truth_val = np.sum(ground_truth)

        #Calculate recall error
        recall_error = self.get_total_level_of_alignment(
            ts=ground_truth,
            other=forecast,
        ...
        precision_error = self.get_total_level_of_alignment(
            ts=forecast,
            other=ground_truth,
            number_of_masks_to_use=number_of_masks_to_use,
            total_ground_truth_val=total_ground_truth_val,
